...
//            dd($response);
            if (empty($response)) {
                ...
                if ($response['code'] === 200)
                    ...
                    echo json_encode(['status' => 'failed', 'code' => '201', 'message' => 'checkpoint required', 'data' => $response], true);
                else
    ...
    public function addInstagramAccountMain(Request $request)
    {
        $postData = $request->all();
        $rules['username'] = 'required|unique:instagram_users,username';
        $rules['password'] = 'required';
        ...
        if (!$validator->fails()) {
            ...
            if (empty($response)) {
                ...
                if ($response['code'] === 200) {
                    ...
                    echo json_encode(['code' => 431, 'status' => 'failed', 'message' => 'Please verify your account'], true);
                } else {
                    echo json_encode($response, true);
                }
            }
    ...
    public function verifyInstagramAccount(Request $request)
    {
        $postData = $request->all();
        $rules['username'] = 'required|unique:instagram_users,username';
        $rules['password'] = 'required';
        ...
        $validator = Validator::make($postData, $rules, $message);
        if (!$validator->fails()) {
            $api_url = $this->API_URL . '/resolve/challenge';
            $data['username'] = $postData['username'];
            $data['password'] = $postData['password'];
            $data['security_code'] = $postData['security_code'];
            ...
            if (empty($response)) {
                ...
                if ($response['code'] === 200) {
                    ...
                    echo json_encode(['code' => 402, 'status' => 'failed', 'message' => 'Please check the code we sent you and try again.'], true);
                } else {
                    echo json_encode(['code' => 400, 'status' => 'failed', 'message' => 'Failed to Add this account, try adding another account.'], true);
                }
//                else if ($response['code'] === 431){
//                    echo json_encode(['code'=>431,'status' => 'failed', 'message' => 'Please verify your account'], true);
//                }
